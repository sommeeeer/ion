...
export function createServersAndDistribution(
  ...
  return all([outputPath, plan]).apply(([outputPath, plan]) => {
    ...
    function createHttpBasicAuthFunction() {
      const basicAuth = Buffer.from(`${args.httpBasicAuth?.username}:${args.httpBasicAuth?.password}`).toString("base64");

      return `if (request?.headers?.authorization?.value !== 'Basic ${basicAuth}') {
        return {
          statusCode: 401,
...
function handler(event) {
  ...
  ${args.httpBasicAuth ? createHttpBasicAuthFunction() : ""}
  ${injections.join("\n")}
  return request;
}`,
            },
